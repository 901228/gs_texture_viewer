#include "texture.hpp"

#include <filesystem>
#include <fstream>
#include <memory>
#include <optional>
#include <stdexcept>

#include <glad/gl.h>

#include <ImGui/imgui.h>

#include <stb/stb_image.h>
#include <toml++/toml.hpp>

#include "../gl/program.hpp"
#include "../logger.hpp"
#include "utils/utils.hpp"

int TextureWrap::gl(Mode mode) {
  switch (mode) {
  case Mode::Repeat:
    return GL_REPEAT;
  case Mode::Mirror:
    return GL_MIRRORED_REPEAT;
  case Mode::Clamp:
    return GL_CLAMP_TO_EDGE;
  case Mode::Border:
    return GL_CLAMP_TO_BORDER;
  default:
    throw std::runtime_error("Unknown texture wrap mode!");
  }
}

std::unique_ptr<ImageTexture> ImageTexture::create(const std::string &path, ColorType colorType,
                                                   TextureWrap::Mode wrapX, TextureWrap::Mode wrapY) {
  unsigned int id;
  float width, height;
  if (!loadImage(path, id, width, height, wrapX, wrapY, colorType)) {
    return nullptr;
  }

  return std::make_unique<ImageTexture>(path, id, width, height, wrapX, wrapY, colorType);
}

ImageTexture::ImageTexture(const std::string &path, const unsigned int &id, const float &width,
                           const float &height, TextureWrap::Mode wrapX, TextureWrap::Mode wrapY,
                           ColorType colorType)
    : _path(path), _name(Utils::File::stem(path)), _id(id), _width(width), _height(height), _wrapX(wrapX),
      _wrapY(wrapY), _colorType(colorType) {}

ImageTexture::~ImageTexture() { glDeleteTextures(1, &_id); }

bool ImageTexture::uploadToGL(const unsigned char *data, int w, int h, int channels, unsigned int &id,
                              float &width, float &height, TextureWrap::Mode wrapX, TextureWrap::Mode wrapY,
                              ColorType &colorType) {

  GLint format, internalformat;
  if (channels == 4) {
    colorType = ColorType::RGBA;
    format = GL_RGBA;
    internalformat = GL_RGBA;
  } else if (channels == 3) {
    colorType = ColorType::RGB;
    format = GL_RGB;
    internalformat = GL_RGB;
  } else if (channels == 1) {
    colorType = ColorType::R;
    format = GL_RED;
    internalformat = GL_R8;
  } else {
    ERROR("Unsupported image with channel count: {}", channels);
    return false;
  }

  glBindTexture(GL_TEXTURE_2D, id);
  // Single-channel rows are tightly packed; the default unpack alignment of 4
  // would over-read when the width is not a multiple of 4.
  glPixelStorei(GL_UNPACK_ALIGNMENT, channels == 1 ? 1 : 4);
  glTexImage2D(GL_TEXTURE_2D, 0, internalformat, w, h, 0, format, GL_UNSIGNED_BYTE, data);
  glGenerateMipmap(GL_TEXTURE_2D);

  width = static_cast<float>(w);
  height = static_cast<float>(h);

  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, TextureWrap::gl(wrapX));
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, TextureWrap::gl(wrapY));
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  float border[] = {0.f, 0.f, 0.f, 0.f};
  glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, border);

  return true;
}

bool ImageTexture::loadImage(const std::string &path, unsigned int &id, float &width, float &height,
                             TextureWrap::Mode wrapX, TextureWrap::Mode wrapY, ColorType &colorType) {

  unsigned char *data = nullptr;
  try {
    glGenTextures(1, &id);

    int w, h, nrComponents, desired_channels = static_cast<int>(colorType);
    stbi_set_flip_vertically_on_load(true);
    data = stbi_load(path.c_str(), &w, &h, &nrComponents, desired_channels);

    if (data) {

      // When desired_channels is non-zero, stb returns exactly that many channels
      // regardless of the file's original channel count (which nrComponents reports).
      // Pick the GL format from the actual channel count of the returned data,
      // otherwise glTexImage2D reads past the end of the buffer and crashes.
      int channels = desired_channels != 0 ? desired_channels : nrComponents;

      if (!uploadToGL(data, w, h, channels, id, width, height, wrapX, wrapY, colorType))
        throw std::runtime_error(std::format("Unsupported image with channel count: {}", channels));

      stbi_image_free(data);
      return true;
    } else {
      throw std::runtime_error(std::format("Failed to load image file {}", path));
    }
  } catch (const std::exception &err) {
    stbi_image_free(data);
    ERROR("Failed to load texture: {}", err.what());
  }

  return false;
}

bool ImageTexture::loadImageFromMemory(const unsigned char *buffer, int length, unsigned int &id,
                                       float &width, float &height, TextureWrap::Mode wrapX,
                                       TextureWrap::Mode wrapY, ColorType &colorType) {

  unsigned char *data = nullptr;
  try {
    glGenTextures(1, &id);

    int w, h, nrComponents, desired_channels = static_cast<int>(colorType);
    // assimp normalizes glTF's top-left UVs to its OpenGL/bottom-left convention (it flips V),
    // so embedded textures must be flipped on load to match (same as the on-disk path).
    stbi_set_flip_vertically_on_load(true);
    data = stbi_load_from_memory(buffer, length, &w, &h, &nrComponents, desired_channels);

    if (data) {
      int channels = desired_channels != 0 ? desired_channels : nrComponents;

      if (!uploadToGL(data, w, h, channels, id, width, height, wrapX, wrapY, colorType))
        throw std::runtime_error(std::format("Unsupported image with channel count: {}", channels));

      stbi_image_free(data);
      return true;
    } else {
      throw std::runtime_error("Failed to decode embedded image buffer");
    }
  } catch (const std::exception &err) {
    stbi_image_free(data);
    ERROR("Failed to load embedded texture: {}", err.what());
  }

  return false;
}

std::unique_ptr<ImageTexture> ImageTexture::createFromMemory(const std::string &name,
                                                             const unsigned char *buffer, int length,
                                                             ColorType colorType, TextureWrap::Mode wrapX,
                                                             TextureWrap::Mode wrapY) {
  unsigned int id;
  float width, height;
  if (!loadImageFromMemory(buffer, length, id, width, height, wrapX, wrapY, colorType)) {
    return nullptr;
  }

  return std::make_unique<ImageTexture>(name, id, width, height, wrapX, wrapY, colorType);
}

void ImageTexture::setupUniforms(const Program &program, unsigned int index, std::string location) const {

  glActiveTexture(GL_TEXTURE0 + index);
  glBindTexture(GL_TEXTURE_2D, _id);
  program.setInt((!location.empty() ? location : std::format("tex[{}]", index)).c_str(),
                 static_cast<int>(index));
}

void ImageTexture::saveTextureList(const std::vector<std::unique_ptr<ImageTexture>> &list,
                                   const std::string_view &filepath) {

  toml::table tbl;
  if (std::filesystem::exists(filepath)) {
    try {
      auto result = toml::parse_file(filepath);
      tbl = std::move(result);
    } catch (const std::exception &err) {
      ERROR("Parsing failed: {}", err.what());
    }
  }

  toml::array texturesList;
  for (const auto &texture : list) {
    texturesList.push_back(texture->path());
  }

  tbl.insert_or_assign("texturesList", texturesList);

  std::ofstream file(std::string(filepath).c_str());
  file << tbl;
  file.close();
}

std::vector<std::unique_ptr<ImageTexture>> ImageTexture::loadTextureList(const std::string_view &filepath) {

  try {
    std::vector<std::unique_ptr<ImageTexture>> result;

    toml::table tbl = toml::parse_file(filepath);

    toml::array *arr = tbl["texturesList"].as_array();
    if (!arr) {
      throw std::runtime_error("texturesList is not an array");
    }

    for (const auto &elem : *arr) {
      if (std::optional<std::string> path = elem.value<std::string>(); path) {
        result.push_back(ImageTexture::create(path.value()));
      }
    }

    return result;
  } catch (const std::exception &err) {
    ERROR("Parsing failed: {}", err.what());
  }
  return {};
}

PBRTexture::PBRTexture(const std::string path, std::string basecolorPath, std::string normalPath,
                       std::string heightPath, std::string roughnessPath, std::string maskPath,
                       float heightScale)
    : _path(path), _name(Utils::File::stem(path)), _heightScale(heightScale) {

  auto directoryPath = std::filesystem::path(path);
  _basecolor = ImageTexture::create((directoryPath / basecolorPath).string());
  _normal = ImageTexture::create((directoryPath / normalPath).string(), ImageTexture::ColorType::RGB);
  _height = ImageTexture::create((directoryPath / heightPath).string(), ImageTexture::ColorType::R);
  _roughness = ImageTexture::create((directoryPath / roughnessPath).string(), ImageTexture::ColorType::R);
  _mask = ImageTexture::create((directoryPath / maskPath).string(), ImageTexture::ColorType::R);

  assert(_basecolor->colorType() == ImageTexture::ColorType::RGBA ||
         _basecolor->colorType() == ImageTexture::ColorType::RGB);
  assert(_normal->colorType() == ImageTexture::ColorType::RGB);
  assert(_height->colorType() == ImageTexture::ColorType::R);
  assert(_roughness->colorType() == ImageTexture::ColorType::R);
  assert(_mask->colorType() == ImageTexture::ColorType::R);
}

PBRTexture::~PBRTexture() = default;

void PBRTexture::controls() {
  ImGui::SliderFloat("roughness scale", &_roughnessScale, 0.0f, 2.0f);

  ImGui::Combo("Height Mode", reinterpret_cast<int *>(&_heightMode),
               "None\0Parallax Occlusion\0Tessellation Displacement\0");

  if (_heightMode != HeightMode::None)
    ImGui::SliderFloat("height scale", &_heightScale, 0.0f, 0.5f);

  if (_heightMode == HeightMode::TessellationDisplacement) {
    ImGui::SliderInt("tessellation level", &_tessLevel, 1, 64);
    ImGui::Checkbox("invert displacement", &_invertHeight);
  }
}

void PBRTexture::setupUniforms(const Program &program, unsigned int index,
                               const PBRTextureLocation &location) const {

  glActiveTexture(GL_TEXTURE0 + index);
  glBindTexture(GL_TEXTURE_2D, _basecolor->id());
  program.setInt(location.basecolor.c_str(), index);

  glActiveTexture(GL_TEXTURE1 + index);
  glBindTexture(GL_TEXTURE_2D, _normal->id());
  program.setInt(location.normal.c_str(), index + 1);

  glActiveTexture(GL_TEXTURE2 + index);
  glBindTexture(GL_TEXTURE_2D, _height->id());
  program.setInt(location.height.c_str(), index + 2);

  glActiveTexture(GL_TEXTURE3 + index);
  glBindTexture(GL_TEXTURE_2D, _mask->id());
  program.setInt(location.mask.c_str(), index + 3);

  program.setFloat(location.heightScale.c_str(), heightScale());

  program.setInt("heightMode", static_cast<int>(_heightMode));
  program.setInt("heightInvert", _invertHeight ? 1 : 0);

  // Only subdivide when actually displacing geometry; other modes stay at 1
  // (a single patch = the original triangle).
  const float tess =
      _heightMode == HeightMode::TessellationDisplacement ? static_cast<float>(_tessLevel) : 1.0f;
  program.setFloat("tessLevel", tess);
}

void PBRTexture::saveTextureList(const std::vector<std::unique_ptr<PBRTexture>> &list,
                                 const std::string_view &filepath) {

  toml::table tbl;
  if (std::filesystem::exists(filepath)) {
    try {
      auto result = toml::parse_file(filepath);
      tbl = std::move(result);
    } catch (const std::exception &err) {
      ERROR("Parsing failed: {}", err.what());
    }
  }

  // re-create the array
  toml::array pbrList;
  for (const auto &pbr : list) {
    toml::table entry;
    entry.insert_or_assign("path", pbr->_path);
    entry.insert_or_assign("basecolor", Utils::File::filename(pbr->_basecolor->path()));
    entry.insert_or_assign("normal", Utils::File::filename(pbr->_normal->path()));
    entry.insert_or_assign("height", Utils::File::filename(pbr->_height->path()));
    entry.insert_or_assign("roughness", Utils::File::filename(pbr->_roughness->path()));
    entry.insert_or_assign("mask", Utils::File::filename(pbr->_mask->path()));
    entry.insert_or_assign("heightScale", pbr->_heightScale);
    pbrList.push_back(entry);
  }

  tbl.insert_or_assign("pbrList", pbrList);

  std::ofstream file(std::string(filepath).c_str());
  file << tbl;
  file.close();
}

std::vector<std::unique_ptr<PBRTexture>> PBRTexture::loadTextureList(const std::string_view &filepath) {

  try {
    std::vector<std::unique_ptr<PBRTexture>> result;

    toml::table tbl = toml::parse_file(filepath);

    toml::array *arr = tbl["pbrList"].as_array();
    if (!arr) {
      throw std::runtime_error("pbrList is not an array");
    }

    for (const auto &elem : *arr) {
      const auto *entry = elem.as_table();
      if (!entry)
        continue;

      auto path = entry->get_as<std::string>("path");
      auto basecolor = entry->get_as<std::string>("basecolor");
      auto normal = entry->get_as<std::string>("normal");
      auto height = entry->get_as<std::string>("height");
      auto roughness = entry->get_as<std::string>("roughness");
      auto mask = entry->get_as<std::string>("mask");
      auto heightScale = entry->get_as<double>("heightScale");

      if (!basecolor || !normal || !height)
        continue; // continue if any of the fields is missing

      if (heightScale) {
        result.push_back(std::make_unique<PBRTexture>(
            path->get(), Utils::File::filename(basecolor->get()), Utils::File::filename(normal->get()),
            Utils::File::filename(height->get()), Utils::File::filename(roughness->get()),
            Utils::File::filename(mask->get()), static_cast<float>(heightScale->get())));
      } else {
        result.push_back(std::make_unique<PBRTexture>(
            path->get(), Utils::File::filename(basecolor->get()), Utils::File::filename(normal->get()),
            Utils::File::filename(height->get()), Utils::File::filename(roughness->get()),
            Utils::File::filename(mask->get())));
      }
    }

    return result;
  } catch (const std::exception &err) {
    ERROR("Parsing failed: {}", err.what());
  }
  return {};
}
